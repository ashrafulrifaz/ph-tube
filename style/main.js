let loadData = async () => {
   ...
   tabItem.forEach(item => {
      ...
      btn.innerHTML = `<button onclick="loadPosts('${item.category_id}')" class="text-sm py-2 px-5 rounded bg-[#25252533] font-medium text-[#252525]">${item.category}</button>`;
...
let allPosts = null;
const loadPosts = async (category_id=1000)=>{
...
function shortData(){
   allPosts.sort(
      (a, b) => b.others.views.split("K")[0] - a.others.views.split("K")[0]
   );
   showPosts(allPosts);
}